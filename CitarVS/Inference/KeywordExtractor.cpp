...
// The code is just here for Fun and the Author is no where obligated to any issue arising from using this code anywhere. The code 
// has not been thoroughly tested for all possible scenarios and for all test conditions.
// The code is inspired by http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.4.5601&rep=rep1&type=pdf
// and doesn't implement all optimizations and possibilities mentioned in the paper. It is just a proof of concept to validate the 
// paper for fun.
//---------------------------------------------------------------------------------------------------------------------------------
#include "stdafx.h"
...
std::vector<std::string> KeywordExtractor::ApplyEndMarkers(std::string rawSentence)
{
	...
	//just get sentences and collect them (very raw sentence extraction)
...
//-----------------------------------------------------------------------------------------------------------------
// Method that extracts the metadata from the sentences
//-------------------------------------------------------------------------------------------------------------------
void KeywordExtractor::SentenceMetaExtract(long sentenceId) 
{
	...
	//total term count in the sentence being tracked here
	long sentenceTermCount = 0;

	for(auto pair : sentence)
	{
		//for any words that are candidate words for us (as NOUNS, VERBS etc)
		if(this->IsCandidateWord(pair))
		{
			...
			//Store the sentence count for each word in the dictionary
			...
			//Word Counts for the given word, total count tracker
	...
	{
		//the term count in the sentences where this particular candidate term existed
		...
		{
			//computation to see when pair of words co exist and maintain there count
...
//Defining a pair and its identity
...
//just maintaining the terms and their symbol Ids
...
{
	//Only choose certain keyword types to be candidates for keywords
	return pair.second == "VBG" || pair.second == "NN" 	|| pair.second == "NP" 	|| pair.second == "NNS" ? true : false;
}

//Main method that is called to extract keywords out of the text
std::vector<std::string> KeywordExtractor::ExtractKeywords(std::string& text)
{
	...
	for(auto sentence : rawSentences) {
		...
		//apply the markers just needed for the tagger in the Citar code base
		...
		//Extract complete metadata out of all sentences
...
void KeywordExtractor::IdentifyCardinalWords()
{
	...
	//compute chi values on high freq words and candidate words bias
